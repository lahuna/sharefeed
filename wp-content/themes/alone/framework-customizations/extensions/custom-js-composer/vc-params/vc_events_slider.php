...
class vcEventsSlider extends WPBakeryShortCode {
    ...
    public function variables($event_id, $item_data) {
      ...
      $variables = array(
        ...
        '{venue}'             => $venue,
        '{day_big}'           => (!empty($event_start_time))?date("d", strtotime($event_start_time)):'',
        '{month_big}'           => (!empty($event_start_time))?date("F Y", strtotime($event_start_time)):'',
      );
	          ...
	          $output = implode('', array(
	            '<div class="item-inner layout-{layout}">',
					'<div class="icon-calendar">',
	                    '<i class="linecons linecons-calendar"></i>',
	                '</div>',
	                '<div class="content-entry">',
